...
import { getOffice,deleteOffice } from '../../action/OfficeActions';
import { Button, Grid } from '@material-ui/core';
import { Link } from "react-router-dom";
import { getOfficeById } from '../../action/OfficeActions';
...
const Office = () => {
    ...
    const loadOffice = async () => {
        const result = await axios.get(`http://localhost:9191/api/ocma/Office/getOffice/${officeId}`).catch((err) => { console.log("Error ", err); });
        dispatch(getOffice(result.data));
